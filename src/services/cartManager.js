...
class CartManager {
    ...
    async createCart() {
        try {
            const newCart = {
                ...
                products: [], // Inicializar productos como un array vacío
            };
    ...
    async getCartById(id) {
        try {
            ...
            return cart; // Retornar directamente el carrito
        } catch (error) {
    ...
    async addProductToCart(cartId, productId) {
        try {
            ...
            if (productInCart) {
                productInCart.quantity += 1; // Aumentar la cantidad si el producto ya está en el carrito
            } else {
                cart.products.push({ product: productId, quantity: 1 }); // Agregar un nuevo producto
            }
